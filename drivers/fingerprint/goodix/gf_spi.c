...
#define KEY_LEFT_ZUK        254
#define KEY_RIGHT_ZUK       249
...
struct wake_lock goodix_wake_lock;
        ...
        for(i = 0; i< ARRAY_SIZE(key_map); i++) {
            if(key_map[i].val == gf_key.key){
                if (gf_key.key == KEY_HOME)
                    gf_key.key =  KEY_NAVI_LONG;  //Convert to U-touch long press keyValue
                input_report_key(gf_dev->input, gf_key.key, 1);
                input_sync(gf_dev->input);
                mdelay(30);
                input_report_key(gf_dev->input, gf_key.key, 0);
                input_sync(gf_dev->input);
                break;
            }
        }
...
static irqreturn_t gf_irq(int irq, void *handle)
{
	...
	{
		smp_rmb();
		wake_lock_timeout(&goodix_wake_lock, 3*HZ);
...
#endif
		wake_lock_init(&goodix_wake_lock, WAKE_LOCK_SUSPEND, "goodix_wakelock");
