...
#define PINCTRL_STATE_ACTIVE	"ts_active"
#define PINCTRL_STATE_SUSPEND	"ts_suspend"
#define PINCTRL_STATE_RELEASE	"ts_release"

#ifdef TOUCH_SYS
static atomic_t ft_device_count;
#endif
int touch_gesture;
static int touch_log_on;
/*******************************************************************************
...
void fts_irq_enable(struct fts_ts_data *data)
{
...
void fts_irq_disable(struct fts_ts_data *data)
{
...
//		x = 1080 - event->au16_x[i];
//		y = 1920 - event->au16_y[i];	
		if(touch_log_on)
...
//	#endif
	printk("touch suspend\n");
		...
		if(touch_gesture){
		  ...
		  {
		  	fts_write_reg(fts_i2c_client, 0xd1, 0x10);
...
int fts_ts_resume(struct device *dev)
{
	...
	#if FTS_GESTURE_EN
		if(touch_gesture){
	        fts_write_reg(fts_i2c_client, 0xd0, 0x0);
		err = disable_irq_wake(data->client->irq);
		if (err)
			dev_err(&data->client->dev,
				"%s: set_irq_wake failed\n", __func__);
			...
			dev_err(&client->dev, "%s:[FTS] create fts control iic driver failed\n",	__func__);
               ...
               //add tp class to show tp info
                ...
                if (IS_ERR(ts->tp_class))
                {
     	           ...
     	           atomic_set(&ft_device_count, 0);
 	        ts->index = atomic_inc_return(&ft_device_count);
