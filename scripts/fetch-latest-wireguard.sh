...
done < <(curl -A "$USER_AGENT" -LSs --connect-timeout 5 https://build.wireguard.com/distros.txt)
...
curl -A "$USER_AGENT" -LsS --connect-timeout 5 "https://git.zx2c4.com/WireGuard/snapshot/WireGuard-$VERSION.tar.xz" | tar -C "net/wireguard" -xJf - --strip-components=2 "WireGuard-$VERSION/src"
