...
/**
 * FIXME: drv2605l has a hw bug, need to remove battery
 * power to recovery it. drv260x_do_notify_work sent uevent to userspace
 * userspace let phone enter into ship mode which battery power supply
 * will be switch off
 **/
static void drv260x_do_notify_work(struct work_struct *notify_work)
{
	struct drv260x_data *haptics = container_of(notify_work, struct drv260x_data, notify_work.work);
	struct i2c_client *client = haptics->client;

	dev_err(&client->dev,
				"I2c error, notify userspace.\n");
	kobject_uevent(&client->dev.kobj, KOBJ_CHANGE);
}

static void drv260x_do_notify(struct drv260x_data *haptics, int delay)
{
	static bool notify_once;
	if (!notify_once) {
		schedule_delayed_work(&haptics->notify_work, delay*HZ);
		notify_once = true;
	}
}
...
static void drv260x_worker(struct work_struct *work)
{
	...
	if(!haptics->magnitude){
		...
		if (wake_lock_active(&haptics->wakelock)){
			wake_unlock(&haptics->wakelock);
		}
		return;
	}

	if(!wake_lock_active(&haptics->wakelock)){
		wake_lock(&haptics->wakelock);
	}
...
error_return:
	drv260x_do_notify(haptics, 1);//1s
...
static int drv260x_do_hw_check(struct drv260x_data *haptics)
{
	...
	if (error) {
		dev_err(&haptics->client->dev,
				...
				error);
		gpio_set_value(haptics->enable_gpio, 0);
		drv260x_do_notify(haptics, 60*3); //3min
		return error;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	INIT_DELAYED_WORK(&haptics->notify_work, drv260x_do_notify_work);
	...
	wake_lock_init(&haptics->wakelock, WAKE_LOCK_SUSPEND,
				"drv260x");
